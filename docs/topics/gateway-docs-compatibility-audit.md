...
### 3.1 Load-aware routing + cluster load stats — **PARTIALLY RESOLVED**
...
- **Cluster load stats (the M7 / UC-MON-02 stats surface) — RESOLVED (Phase 12, Tasks 75–80).** A config-selectable cluster-stats collector (`clusterStats.monitorType` ∈ `INFO_API` default / `NOOP` / `UI_API` / `METRICS`) now collects live running/queued counts (plus `numWorkerNodes` and per-user `userQueuedCount` under `UI_API`). It rides the health-monitor tick into a name-keyed `internal/clusterstats` store, surfaced by the `getAllBackends` UI table and the M7 public backend-state endpoint (see 3.x below). `INFO_API` (default) issues no extra HTTP and reports counts of 0 — byte-for-byte the Java default; `JDBC`/`JMX` are v1-deferred (rejected at startup, SCOPE §2).
- **Load-aware *routing strategy* (`QueryCountBasedRouter`) — STILL GAP (by design).** Intra-group cluster selection remains the responsibility of the external routing service (trino-goway only supports `EXTERNAL` routing). The stats above are reported/observable but are not yet consumed by an in-gateway least-loaded selector. Operators relying on `QueryCountBasedRouter` implement it in their external router.
...
### 3.6a Public backend-state wire shape (M7 / UC-ADM-14) — **RESOLVED (Phase 12, Task 79)**
`gateway-api.md`: `GET /api/public/backends/{name}/state` is documented to return Java's `ClusterStats` record, not the health-only `BackendResponse` Go previously served.
- **RESOLVED:** the endpoint now emits the M7 `ClusterStatsResponse` 9-field shape — `{clusterId, runningQueryCount, queuedQueryCount, numWorkerNodes, trinoStatus, proxyTo, externalUrl, routingGroup, userQueuedCount}` — with field names matching Java exactly. Counts are live under the `UI_API`/`METRICS` collectors and 0 under the `INFO_API` default (3.1). Two intentional divergences from Java are documented in `docs/trino-goway-use-cases-comparison.md` and SCOPE §1: (a) `trinoStatus` uses the shared `internal/clusterstatus` enum with a new `UNKNOWN` member (the admin label no longer collapses Unknown→PENDING); (b) the not-yet-collected default object is populated from the persistence row (`proxyTo`/`externalUrl`/`routingGroup`) rather than Java's raw null, with `userQueuedCount` staying null until a `UI_API` collection fills it.
