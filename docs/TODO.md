...
- [x] `internal/config/config.go` — new `ClusterStatsConfig{ MonitorType string }` (`yaml:"monitorType"`); top-level node `clusterStats:` wired as `Config.ClusterStats` — config.go:99 `ClusterStatsConfig`, wired Config.ClusterStats config.go:91
- [x] `internal/config/config.go` — new `BackendStateConfig{ Username, Password string; SSL bool; XForwardedProtoHeader bool }` (`yaml:"username"/"password"/"ssl"/"xForwardedProtoHeader"`); top-level node `backendState:` wired as `Config.BackendState` — config.go:106 `BackendStateConfig`, wired Config.BackendState config.go:92
- [x] `internal/config/config.go` — extend `MonitorConfig` with `StatsTimeout Duration`, `Retries int`, `MetricsEndpoint string`, `RunningQueriesMetricName string`, `QueuedQueriesMetricName string`, `MetricMinimumValues map[string]float64`, `MetricMaximumValues map[string]float64` (matching yaml keys) — config.go:135-148
- [x] `internal/config/config.go::defaultConfig`/`applyDefaults` — seed: `MonitorType→INFO_API`; `StatsTimeout→10s` (Java queryTimeout), `Retries→0`, `MetricsEndpoint→/metrics`, `RunningQueriesMetricName→trino_execution_name_QueryManager_RunningQueries`, `QueuedQueriesMetricName→trino_execution_name_QueryManager_QueuedQueries`, `MetricMinimumValues→{trino_metadata_name_DiscoveryNodeManager_ActiveNodeCount:1}` (when omitted — NOT empty), `MetricMaximumValues→{}` — defaultConfig config.go:233-247, applyDefaults config.go:295-323 (nil-guard preserves explicit `{}`)
- [x] `internal/config/config.go::Validate` — accept `monitorType ∈ {NOOP, INFO_API, UI_API, METRICS}`; reject `JDBC`/`JMX` with `config: validate: clusterStats.monitorType %q not supported in v1` (R8); require non-empty `backendState.username` when `monitorType ∈ {UI_API, METRICS}`; validate `statsTimeout > 0`, `retries >= 0` — validateClusterStats config.go:430
- [x] `internal/config/config_test.go` — table-driven: YAML round-trip; defaults applied when omitted (esp. metric names + min-values map); JDBC/JMX rejected; UI_API/METRICS without `backendState.username` rejected; INFO_API + empty backendState accepted — TestConfig_ClusterStats_* in config_test.go
- [x] `go vet ./...` + `golangci-lint run ./...` pass
...
- [x] `internal/clusterstatus/doc.go` + `status.go` — `Status int` with members `Unknown, Healthy, Unhealthy, Pending`; `Label() string` → `"HEALTHY"|"UNHEALTHY"|"PENDING"|"UNKNOWN"` (admin wire form) + lowercase `String()` for logs; NO internal imports — clusterstatus/doc.go, status.go (Label/String incl. UNKNOWN)
- [x] `internal/monitor/status.go` — replace the local enum with aliases: `type TrinoStatus = clusterstatus.Status` and `StatusUnknown/Healthy/Unhealthy/Pending` consts → `clusterstatus.*` so `internal/monitor` + all `monitor.Status*` consumers (admin) compile unchanged — monitor/status.go: TrinoStatus = clusterstatus.Status, Status* consts aliased
- [x] `internal/clusterstats/doc.go` + `stats.go` — `ClusterStats{ClusterID, RunningQueryCount, QueuedQueryCount, NumWorkerNodes int; TrinoStatus clusterstatus.Status; ProxyTo, ExternalURL, RoutingGroup string; UserQueuedCount map[string]int}` — clusterstats/doc.go, stats.go
- [x] `internal/clusterstats/collector.go` — `Backend` interface ({GetName, GetURL, GetExternalURL, GetRoutingGroup}); `Collector` interface (`Collect(ctx, b Backend) ClusterStats`); `NewCollector(cfg config.ClusterStatsConfig, mon config.MonitorConfig, bs config.BackendStateConfig, monitorStatus func(url string) clusterstatus.Status, httpClient *http.Client, log *slog.Logger) (Collector, error)` — selector switch over `NOOP|INFO_API|UI_API|METRICS`, default INFO_API, any other (incl. JDBC/JMX) → error (defense-in-depth) — clusterstats/collector.go: Backend/Collector/NewCollector selector, default INFO_API, JDBC/JMX/unknown→error
- [x] `internal/clusterstats/store.go` — `StatsStore` (`atomic.Pointer[map[string]ClusterStats]` keyed by NAME); `ObserveStats(map[string]ClusterStats)` (write, single swap) + `Stats(name string) ClusterStats` (read; zero `ClusterStats{ClusterID:name}` for unknown, never error — R7). Top-of-file comment: clusterstats must NOT import monitor (one-way) — clusterstats/store.go: atomic.Pointer name-keyed, ObserveStats/Stats, one-way note at top
...
- [x] `internal/clusterstats/noop.go` — `Collect` returns `ClusterStats{ClusterID, ProxyTo, ExternalURL, RoutingGroup from b, TrinoStatus: monitorStatus(b.GetURL())}`; no HTTP; counts 0 — clusterstats/noop.go
- [x] `internal/clusterstats/infoapi.go` (DEFAULT) — `TrinoStatus` ONLY, reusing the monitor verdict via `monitorStatus(url)` — issues NO extra HTTP; ProxyTo/ExternalURL/RoutingGroup from `b`; counts 0 — clusterstats/infoapi.go (no HTTP, reuses StatusFunc)
- [x] `internal/clusterstats/uiapi.go` — per-collector `http.CookieJar` inside the collector (R5); `POST {proxyTo}/ui/login` form `username`+`password` (one session reused across ticks — documented optimization over Java's fresh-login-per-GET); `GET /ui/api/stats` → `activeWorkers`(→NumWorkerNodes), `queuedQueries`, `runningQueries`; `TrinoStatus = activeWorkers>0 ? Healthy : Unhealthy`; `GET /ui/api/query?state=QUEUED` → tally `sessionUser` into `UserQueuedCount`; `xForwardedProtoHeader` adds `X-Forwarded-Proto: https` on the TWO GETs (not login); 401/empty → partial, no counts, no panic — clusterstats/uiapi.go (login-once jar reuse, 401→invalidate)
- [x] `internal/clusterstats/metrics.go` — per required metric `GET {proxyTo}{metricsEndpoint}?name[]=<metric>`; auth `Authorization: Basic base64(user:pass)` if password else `X-Trino-User: <username>`; `Content-Type: application/openmetrics-text; version=1.0.0; charset=utf-8`; parse (split `\n`, drop `#`, `SplitN(line," ",2)`, parseFloat→int truncation); any required metric absent → UNHEALTHY; gating: empty/min-absent-or-below/max-absent-or-above → UNHEALTHY else HEALTHY; no NumWorkerNodes/UserQueuedCount — clusterstats/metrics.go (OpenMetrics parse, min/max gate, Basic vs X-Trino-User)
- [x] `internal/clusterstats/{uiapi,metrics}.go` — honor `Monitor.StatsTimeout` (`context.WithTimeout`) + `Monitor.Retries` (retry GET on transport error; backoff via `select`+`ctx.Done`, no `time.Sleep`) — context.WithTimeout + backoff() select/ctx.Done in http.go
...
- [x] `internal/monitor/monitor.go` — `StatsObserver` interface (`ObserveStats(map[string]clusterstats.ClusterStats)`, nil-safe like `StatusObserver`). This is the one place `monitor` imports `clusterstats` (one-way) — monitor.go: StatsObserver (ObserveStats), only place monitor imports clusterstats
- [x] `internal/monitor/monitor.go` — `ClusterStatsCollector` field (`clusterstats.Collector`, nil-safe) + `statsObserver`; setters `SetClusterStatsCollector`/`SetStatsObserver` (call before Start, mirroring `SetObserver`) — monitor.go: collector/statsObserver fields + SetClusterStatsCollector/SetStatsObserver setters
- [x] `internal/monitor/monitor.go::probe` — after the health swap + `notifyObserver()`, when collector + statsObserver non-nil, build a name-keyed snapshot via `collector.Collect(probeCtx, b)` per backend reusing the SAME per-backend fan-out (no second scheduler, R1); hand one map to `statsObserver.ObserveStats(...)` — probe→collectStats(): second fan-out post health-swap, StatsTimeout-bounded, one ObserveStats per tick
- [x] `internal/monitor/monitor.go` — extend `Backend` interface with `GetExternalURL()`/`GetRoutingGroup()`; extend `SimpleBackend` with `ExternalURL`/`RoutingGroup` fields + getters (so the monitor backend satisfies `clusterstats.Backend`) — Backend +GetExternalURL/GetRoutingGroup; SimpleBackend +ExternalURL/RoutingGroup fields+getters
- [x] `internal/monitor/monitor.go::checkOne` — FIX: `info.Starting==true → StatusPending` (currently `StatusUnhealthy` at ~`:315-317`; Java maps `starting→PENDING`); update the doc comment — checkOne returns StatusPending for starting=true (was Unhealthy); doc updated
- [x] `internal/monitor/monitor.go` — readiness/first-tick UNCHANGED: stats must never gate readiness or abort the tick (R7) — collectStats runs after notifyObserver; no effect on m.status/onFirstTick (R7)
- [x] `internal/monitor/monitor_test.go` — collector called once per backend per tick; `ObserveStats` receives a name-keyed snapshot; nil collector/observer → no panic, no stats call; `starting==true` now PENDING (update assertion); readiness still flips on first tick regardless of stats; goleak clean — TestMonitor_StatsCollectorRunsPerTick/_NilStatsHook_NoPanic/_ReadinessNotGatedByStats; StartingBackend asserts Pending; goleak via VerifyTestMain; -race green
- [x] `go vet ./...` + `golangci-lint run ./...` pass
...
- [x] `internal/admin/router.go` — `StatsProvider{ Stats(name string) clusterstats.ClusterStats }`; add optional `Stats StatsProvider` to `admin.Config` (nil ⇒ counts 0) — router.go: StatsProvider + optional Config.Stats (nil⇒0)
- [x] `internal/admin/backend.go::trinoStatusLabel` — delegate to `clusterstatus.Status.Label()` (emits `"UNKNOWN"`); the `default` case must no longer collapse Unknown→PENDING — backend.go: trinoStatusLabel→s.Label() (UNKNOWN no longer collapses to PENDING)
- [x] `internal/admin/backend.go::backendResponseFromPersistence` — populate `Queued`/`Running` from `a.cfg.Stats.Stats(b.Name)` when `Stats != nil` (lookup by NAME); 0 when nil; keep `Status` from `Monitor.Status(b.URL)` — backend.go: Queued/Running from Stats.Stats(b.Name); 0 when nil
- [x] `internal/admin/backend.go` — new wire type `ClusterStatsResponse` (JSON tags `clusterId, runningQueryCount, queuedQueryCount, numWorkerNodes, trinoStatus, proxyTo, externalUrl, routingGroup, userQueuedCount`; `userQueuedCount` `,omitempty` so uncollected default emits null/absent) + mapper `clusterStatsResponseFromPersistence(b, cs)`: `clusterId=b.Name`; counts/numWorkerNodes/userQueuedCount from `cs`; `trinoStatus` from `Monitor.Status(b.URL)` label; proxyTo/externalUrl/routingGroup POPULATED FROM PERSISTENCE (choice b) — backend.go: ClusterStatsResponse (9 tags, userQueuedCount omitempty) + clusterStatsResponseFromPersistence (proxyTo/externalUrl/routingGroup from persistence)
- [x] `internal/admin/backend.go::getPublicBackendState` — emit `ClusterStatsResponse` (M7) instead of `BackendResponse`; nil `Stats` ⇒ zero `ClusterStats{ClusterID:name}` (counts 0, persistence fields still populated); 404 on missing name (unchanged) — backend.go: emits ClusterStatsResponse; nil Stats⇒zero counts, persistence fields populated; 404 unchanged
- [x] `internal/admin/webapp.go::webappGetAllBackends` — call site unchanged; verify Queued/Running are no longer hardcoded 0 (flow from the store) — webapp.go unchanged; counts flow via backendResponseFromPersistence (TestGetAllBackends_LiveCountsFromStore)
- [x] `internal/admin/backend_test.go`/`webapp_test.go` — getAllBackends live counts from a fake StatsProvider keyed by name; nil ⇒ 0; `getPublicBackendState` 9 fields exact tags (golden `testdata/cluster_stats_response.golden`); UNKNOWN → `trinoStatus:"UNKNOWN"`; unobserved default → populated proxyTo/externalUrl/routingGroup, null userQueuedCount, 0 counts — backend_test.go: live/nil counts, 9-field golden testdata/cluster_stats_response.golden, UNKNOWN, unobserved-default; admin_gaps_test.go updated to ClusterStatsResponse
- [x] `go vet ./...` + `golangci-lint run ./...` pass
...
- [x] `cmd/trino-goway/main.go` — 4th `*http.Client` `statsClient` (timeout `Monitor.StatsTimeout`; `CheckRedirect: ErrUseLastResponse`; cookie jar NOT here — lives in the collector per R5); construct ONLY when `ClusterStats.MonitorType ∈ {UI_API, METRICS}` — main.go: statsClient built only for UI_API/METRICS (StatsTimeout, ErrUseLastResponse, no jar)
- [x] `cmd/trino-goway/main.go` — update the "three distinct clients" comment/ledger to FOUR; add `statsClient` (conditionally constructed) to the Hard Invariant #12 ledger — main.go: clients comment now FOUR; statsClient in Hard Invariant #12 ledger
- [x] `cmd/trino-goway/main.go` — `statsStore := clusterstats.NewStatsStore()`; `collector, err := clusterstats.NewCollector(cfg.ClusterStats, cfg.Monitor, cfg.BackendState, mon.Status, statsClient, log...)` (pass `mon.Status` so INFO_API/NOOP reuse the verdict); wrap error — main.go: NewStatsStore + NewCollector(cfg.ClusterStats, cfg.Monitor, cfg.BackendState, mon.Status, statsClient, log) with wrapped error
- [x] `cmd/trino-goway/main.go` — `mon.SetClusterStatsCollector(collector)` + `mon.SetStatsObserver(statsStore)` before `mon.Start`; pass `Stats: statsStore` into `admin.New(...)` — main.go: SetClusterStatsCollector + SetStatsObserver before Start; Stats: statsStore into admin.New
- [x] `cmd/trino-goway/main.go::refreshBackends` — populate `SimpleBackend.ExternalURL`/`RoutingGroup` from persistence so collectors get a fully-populated `clusterstats.Backend` — main.go: SimpleBackend ExternalURL/RoutingGroup from persistence
- [x] `cmd/trino-goway/main.go` — compile-time assertions: `_ monitor.StatsObserver = (*clusterstats.StatsStore)(nil)`, `_ admin.StatsProvider = (*clusterstats.StatsStore)(nil)` — main.go: _ monitor.StatsObserver / _ admin.StatsProvider = (*clusterstats.StatsStore)(nil)
- [x] `go build ./cmd/trino-goway` produces a binary; startup log gains `clusterStatsMonitor = cfg.ClusterStats.MonitorType` — binary builds (23MB); startup log clusterStatsMonitor=cfg.ClusterStats.MonitorType
- [x] `go vet ./...` + `golangci-lint run ./...` pass
...
- [x] `internal/testutil/trino_fake.go` — UI_API: `POST /ui/login` (form creds → `Set-Cookie: Trino-UI-Token=...; HttpOnly; Path=/`, record login; wrong/empty → 403); `GET /ui/api/stats` (require cookie else 401; JSON `{activeWorkers, runningQueries, queuedQueries}`; record `X-Forwarded-Proto`); `GET /ui/api/query?state=QUEUED` (require cookie; JSON array of `{queryId, sessionUser, state}`)
- [x] `internal/testutil/trino_fake.go` — METRICS: `GET /metrics` (configurable path; require Basic or `X-Trino-User`, record which; return configurable OpenMetrics body verbatim)
- [x] `internal/testutil/trino_fake.go` — setters/assertions: `SetUICredentials`, `SetUIStats(activeWorkers,running,queued)`, `SetQueuedQueries(map[string]int)`, `SetMetrics(body)`, `SetMetricsPath`, `LoginCount`, `UIStatsHits`/`UIQueryHits`/`MetricsHits`, `LastForwardedProto`, `LastMetricsAuth() (basicUser, trinoUser string)`
- [x] `internal/testutil/trino_fake_test.go` — `TestTrinoFake_UILogin_SetsCookie`, `_UIStats_RequiresCookie`, `_UIQuery_PerUserQueued`, `_Metrics_AuthHeaderRecorded`, `_Metrics_BodyVerbatim`, `_DefaultsAreInert` (in `trino_fake_clusterstats_test.go`, same package)
- [x] `go vet ./...` + `golangci-lint run ./...` pass
...
- [x] `internal/clusterstatus/status_test.go` — `TestStatus_StringRoundTrip` (HEALTHY|UNHEALTHY|PENDING|UNKNOWN), `_UnknownIsZeroValue`, `_UnmappedLabelFallsBackToUnknown`
- [x] `internal/clusterstats/collector_infoapi_test.go` — `TestInfoAPICollector_ReflectsStatusFunc_NoHTTP` (table over Healthy/Unhealthy/Pending(=starting→PENDING via option-a direct StatusFunc, per lead's Task-78 caveat)/Unknown; asserts zero-count parity + zero-HTTP via recording transport)
- [x] `internal/clusterstats/collector_uiapi_test.go` — `_LoginThenStats`, `_ActiveWorkersZero_Unhealthy`, `_PerUserQueuedTally`, `_LoginFailed_NoCounts`, `_EmptyStats_NoCounts`, `_XForwardedProtoHeader` (enabled/disabled), `_CookieReusedAcrossCalls` (LoginCount==1)
- [x] `internal/clusterstats/collector_metrics_test.go` — `_ParsesOpenMetrics` (truncation 3.9→3), `_MinimumGate_Unhealthy`, `_MissingRequiredMetric_Unhealthy`, `_MaximumGate_Unhealthy`, `_HealthyWhenThresholdsMet` (no numWorkerNodes/userQueuedCount), `_AuthHeaderSelection` (Basic vs X-Trino-User), `_DefaultMinimums`
- [x] `internal/clusterstats/collector_noop_test.go` — `_NeverProbes` (zero/UNKNOWN, recording transport Calls()==0) + `_NilStatusFunc_NoPanic`
- [x] `internal/clusterstats/store_test.go` — `_KeyedByName`, `_UnobservedDefaultFromPersistence` (counts 0, status UNKNOWN, userQueuedCount nil; proxyTo/externalUrl/routingGroup empty at the store, filled at the admin boundary — choice b), `_ObserveReplacesSnapshot`, `_ConcurrentReadWrite` (`-race`)
- [x] `internal/clusterstats/factory_test.go` — `TestNewCollector_SelectsByType` (NOOP/INFO_API/UI_API/METRICS → right concrete type; "" → INFO_API; JDBC/JMX/unknown → error)
- [x] `go vet ./...` + `golangci-lint run ./...` pass (collector helpers in `collector_helpers_test.go`)
...
- [x] `internal/e2e/harness/harness.go` — `WithClusterStats(monitorType, backendStateUsername, backendStatePassword string)` Option renders a `clusterStats:`+`backendState:` block + the `monitor:` stats knobs (metric names + ActiveNodeCount>=1 minimum) into the temp YAML, only when a type is set (empty ⇒ omitted ⇒ INFO_API default, every existing e2e test unchanged). `AddBackend` already returns the `*TrinoFake` so tests call `SetUICredentials`/`SetUIStats`/`SetQueuedQueries`/`SetMetrics`
- [x] `internal/e2e/cluster_stats_e2e_test.go`:
  - [x] `TestE2E_ClusterStats_InfoAPI_PublicStateShape` — 9-field `ClusterStatsResponse` (asserts M7 keys present + BackendResponse keys `status`/`queued`/`running` ABSENT); counts 0; `trinoStatus HEALTHY`
  - [x] `TestE2E_ClusterStats_InfoAPI_GetAllBackendsZeroCounts` — `POST /webapp/getAllBackends` Queued/Running 0
  - [x] `TestE2E_ClusterStats_InfoAPI_StartingIsPending` — `SetStarting(true)` ⇒ public state stays `PENDING` across the probe window, never HEALTHY/UNHEALTHY (starting→PENDING correction)
  - [x] `TestE2E_ClusterStats_UnobservedDefaultFromPersistence` — proxyTo/externalUrl(=proxyTo fallback)/routingGroup populated, counts 0, userQueuedCount absent, missing name still 404, never 500 (choice b)
  - [x] `TestE2E_ClusterStats_UIAPI_LiveCountsSurface` — `WithClusterStats("UI_API","admin","")`; running 5/queued 2/numWorkerNodes 3/userQueuedCount.alice 2 on `.../state` + getAllBackends Queued 2/Running 5
  - [x] `TestE2E_ClusterStats_Metrics_CountsAndThreshold` — ActiveNodeCount 4 ⇒ HEALTHY w/ counts; drop to 0 (below min) ⇒ next tick counts zeroed
  - [x] `TestE2E_ClusterStats_ReadyzNotGatedByStats` (Hard Invariant #12) — UI_API login always rejected (stats fail every tick); backend still reaches HEALTHY + `/trino-gateway/readyz` 200
- [x] `goleak` clean — via the repo's `newHarnessNoLeaks` helper (the e2e package uses per-test `IgnoreCurrent`, not `VerifyTestMain`; consistent with `metrics_e2e_test.go`)
- [x] `go vet -tags e2e ./internal/e2e/...` pass; ran live under Docker (`TRINO_GOWAY_BIN` pinned) — all 7 PASS. NOTE: deadlines set to `backendProbeWait`=30s because main's `backendRefreshInterval`=15s + one probe tick means a newly-added backend takes ~17s to surface — the pre-existing 10s-deadline health tests are latently flaky against this same boundary (flagged to lead)
...
- [x] `cmd/goway-diff-harness/testdata/scenarios/public-backend-state-clusterstats.yaml` — registers a backend via `POST /gateway/backend/modify/add` (the proven admin-backend-crud path), `GET /api/public/backends/{name}/state`, then delete; asserts shape + field-name equivalence (NOT live integer values); `rewriteHostPort: true`. Per-entry `# [JUSTIFIED]` on `runningQueryCount`/`queuedQueryCount`/`numWorkerNodes`/`userQueuedCount`/`clusterId`/`externalUrl`
- [x] `internal/diffharness/testdata/java-gateway-config.yaml.tmpl` — already carries `clusterStatsConfiguration.monitorType: INFO_API` (matches Go's default), so Java emits the `ClusterStats` shape with counts 0; no `bootstrap.go` change needed. Noted in the scenario header
- [x] `cmd/goway-diff-harness/live_test.go` — added to `liveExcludedScenarios` (documented unit-only rationale, mirroring `query-history-scoping`): under INFO_API both sides match field-names + counts 0, but `externalUrl` (Go proxyTo-fallback per choice b vs Java raw null) and `userQueuedCount` (Go omitempty-absent vs Java explicit null) diverge by value; the 9-field shape is asserted positively unit-side (backend_test.go golden) + E2E (`TestE2E_ClusterStats_*`)
- [x] Every `diff.ignore*` carries inline `# [JUSTIFIED]`; passes `TestCommittedScenarios_LoadAndJustified`
- [x] `go vet ./...` + `go vet -tags diff ./...` pass; `golangci-lint run ./internal/diffharness/... ./cmd/goway-diff-harness/...` = 0 issues
...
- [x] `configs/config.example.yaml` — documented `clusterStats:` (`monitorType: INFO_API` + commented UI_API/METRICS/NOOP; note JDBC/JMX v1-deferred), `backendState:` (username/password/ssl/xForwardedProtoHeader), and `monitor:` stats knobs with the Java default values shown — config.example.yaml: monitor stats knobs + clusterStats (INFO_API + commented alts, JDBC/JMX deferred) + backendState; binary smoke-loads it
- [x] `README.md` — "Live cluster stats" subsection: the four monitor types; INFO_API/NOOP issue no extra HTTP (reuse the health probe); the dedicated `statsClient` (4th client) for UI_API/METRICS; the M7 endpoint + 9-field shape — README.md "Live cluster stats": 4 types table, no-extra-HTTP default, statsClient, M7 9-field shape
- [x] `docs/SCOPE.md` §2 — "Cluster-stats monitor types JDBC / JMX (v1-deferred)" note (rejected by `Validate()`; ref R8; gap-closure, no §5 sign-off — lead-confirmed) — SCOPE §2 'Cluster-stats monitor types JDBC / JMX (v1-deferred)' (R8, gap-closure)
- [x] `docs/SCOPE.md` §1 — add "Live queued/running cluster stats (UC-MON-02 / M7)" with the divergence notes: (a) shared `clusterstatus` enum + new `UNKNOWN`; (b) unobserved-default object populated from persistence (not Java null), `userQueuedCount` null until UI_API — SCOPE §1 row w/ divergences (a) shared clusterstatus+UNKNOWN, (b) persistence-populated default
- [x] `docs/PRD.md` — mark cluster-stats monitoring / M7 implemented; note the INFO_API `starting→PENDING` correction — PRD §v1 Scope cluster-stats/M7 + starting→PENDING note; Hard Invariant #7 updated to FOUR clients
- [x] `docs/topics/gateway-docs-compatibility-audit.md` — mark the M7 gap-closure resolved (add a short M7 entry if absent) — audit 3.1 PARTIALLY RESOLVED (stats done, routing-strategy still external) + new 3.6a M7 RESOLVED
- [x] `docs/trino-goway-use-cases-comparison.md` — record the two intentional divergences as prose (b: persistence-populated default; the one-session UI_API optimization vs Java fresh-login-per-GET) so they're not later flagged as accidental gaps (the checkbox flips themselves are Task 87) — comparison doc 'Cluster-stats divergences (Phase 12)' prose: persistence-populated default + one-session UI_API optimization (checkbox flips left for Task 87)
- [x] `go vet ./...` + `golangci-lint run ./...` pass
...
- [x] **UC-MON-02**: `[ ]→[x]` — "✅ live queued/running via config-selectable collector (INFO_API default / UI_API / METRICS). `internal/clusterstats/`. E2E `TestE2E_ClusterStats_*`."
- [x] **UC-ADM-14**: `[ ]→[x]` — "✅ returns `ClusterStatsResponse` (9 fields) — M7 closed; counts live under UI_API/METRICS, 0 under INFO_API; unobserved default populated from persistence."
- [x] **UC-ADM-15**: `[ ]→[x]` — "✅ getAllBackends Queued/Running from the stats store (live under UI_API/METRICS; 0 under INFO_API default)."
- [x] **UC-UI-05**: kept `[x]`, dropped the "⚠️ queued/running tiles read 0 (MON-02)" qualifier — now "queued/running tiles live under UI_API/METRICS (0 under INFO_API default)."
- [x] Wire-shape table: removed the `M7` row and the `queued`/`running` always-`0` row; folded both into the "closed" note (M7 + counts-live bullet)
- [x] Non-goals table: removed the `MON-02` row (now "the 4 ❌"); scoreboard re-tallied (MON 10/10/0/0, ADM 21/5/0; totals 63/14/4) — corrected a pre-existing ADM mis-tally in the process
- [x] Sanity-checked **UC-MON-01**: `{"starting":false} ⇒ HEALTHY` unchanged; only starting=true is PENDING (asserted by `TestE2E_ClusterStats_InfoAPI_StartingIsPending`)
- [x] `go vet ./...` + `golangci-lint run ./...` pass (doc-only change; no Go affected)
