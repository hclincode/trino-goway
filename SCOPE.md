...
**Reference:** `PRD.md`, `topics/do-we-needs-golang-trino-gateway.md`
...
| Admin REST API | All routes and `@RolesAllowed` roles (`ADMIN`/`USER`/`API`) from Java surface; spec from `studies/trino-gateway/admin-api-surface.java-analyst.md` (Task 16) |
...
1. **Written rationale** in a new file under `topics/` documenting the change, operator demand evidence, and implementation cost.
2. **Team-lead acknowledgment** in the git commit message explicitly referencing the `topics/` discussion doc by filename.
