...
**Note:** The architect's perspective is drawn from their 7 completed study documents (`studies/trino-gateway/architecture-overview.architect.md`, `concurrency-model.architect.md`, `library-landscape-go-mapping.md`, `rewrite-hotspots.md`, `config-coupling-depth.architect.md`, `jvm-idioms-not-to-port.md`, `component-build-order.architect.md`) as they were unable to participate directly in the Task #9 discussion.

**Key findings from studies:**
- The ~13 Airlift modules collapse to: `net/http` + `chi` + `slog` + `prometheus/client_golang` + `otel` + a ~100 LOC `internal/lifecycle` package + a ~80 LOC `internal/config/units` package for `DataSize`/`Duration` custom unmarshalers.
- Guice DI is replaced by explicit constructor wiring — Go's compiler-enforced dependency graph is a strict improvement.
- Concurrency model: the Java gateway's JAX-RS async (`@Suspended AsyncResponse`) maps to standard Go `http.Handler` goroutines. The JVM thread-pool model has no direct port — Go goroutines are native.
- Rewrite hotspots: `ProxyRequestHandler` (the proxy core), `BaseRoutingManager` (the 3-step cache-miss chain), and the auth filter chain are the three areas of highest porting complexity.

**Open library divergences requiring resolution before code-write:**
- Cache: `hashicorp/golang-lru/v2` + `singleflight` vs `expirable`/`ristretto` — single-flight matters for DB-backed loaders
- Migration tool: `pressly/goose` vs `golang-migrate/migrate`
- `backendToStatus` map: `sync.RWMutex`+`map` vs `sync.Map` (former is correct for ~10-key, single-writer-many-reader pattern)
- `ActiveClusterMonitor` ticking: interval vs rate semantics (behavior change worth explicit documentation)
